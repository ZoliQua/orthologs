...
print >> sys.stderr, "Using %s compiler" % compiler
...
if compiler == "gnu95":
    ...
    print >> sys.stderr, "Compiler %s not supported. Let's hope things just work." % compiler


blas_info = system_info.get_info("blas_opt")

if len(blas_info) == 0:
    print >> sys.stderr, "No optimised BLAS implementation found. Make sure libopenblas is in the current directory."
    blas_info = {
        "library_dirs": ["."],
        "libraries": ["openblas"]
    }
