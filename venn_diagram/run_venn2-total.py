...
for index, row in data.iterrows():
    key = ''
    for col in range(7, 14):  # Columns from 8 to 14 (index 7 to 13)
        if not pd.isna(row[col]):
            key += chr(65 + col - 7)  # Convert column number to corresponding uppercase letter (A to G)
    if key:
...
for text in texts:
    ...
    if text.string.startswith('Name'):
        ...
        header_name = data.columns[7 + col]
        text.string = header_name

# Write the modified SVG file
output_file = f'output/ortholog_venn_modified_{shapes}_total.svg'
